"""Typed terminal state shared by OpenAI-compatible stream owners.

Async generators cannot return a value to their ``async for`` caller. A
``StreamTerminal`` is therefore passed into the generator: the inner stream
reports what actually happened, and the outer owner records metrics/archive
state exactly once after iteration or cancellation.
"""
