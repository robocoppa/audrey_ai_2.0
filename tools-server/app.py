...
@app.post(
    ...
    description=(
        ...
        "A thin or empty result is NOT proof the content is absent — it means "
        "this search did not surface it. Before telling the user that a file "
        "does not cover something, open that file with get_file_text and look. "
        "Absence is a claim about the file itself, and a search miss is not "
        "evidence for it."
    ),
)
...
@app.post(
    ...
    description=(
        ...
        "transcript', 'what was said start to finish', a long verbatim "
        "passage. "
        "For a question ABOUT a topic, use kb_search instead, even when the "
        "user names a file: 'what did they say about X in Y.mp4' is a search "
        "scoped with kb_search(filename='Y.mp4'), and 'what do my videos say "
        "about X' is an unscoped kb_search. Reading a whole file to answer a "
        "topic question wastes rounds and, because you then summarise across "
        "page boundaries, is where details get mixed up between files. "
        "The ONE exception is checking a negative: if a kb_search came back "
        "thin or empty and you are about to tell the user that a file does not "
        ...
        "how much you have read and how much remains rather than implying you "
        "have all of it. If a transcript is far too long to reproduce in full, "
        "say so and offer to summarise it or find a specific part. "
        "You can always reach the rest of it by calling again with the next "
        "offset, however long it is. Never tell the user to obtain the text "
