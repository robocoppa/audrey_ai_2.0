...
def test_owui_auth_adapter_is_dormant_by_default(monkeypatch):
    monkeypatch.delenv("OWUI_AUTH_ENABLED", raising=False)

    assert EnvOverrides(_env_file=None).owui_auth_enabled is False
    assert EnvOverrides(_env_file=None, OWUI_AUTH_ENABLED="1").owui_auth_enabled is True
