...
def test_writer_system_unchanged():
    expected = (
        ...
        "then answer in two parts: (1) state what you KNOW with confidence from your "
        "training PLAINLY, as established fact. Widely-known, stable facts are fair to "
        "assert even without this session's grounding: a major model's release name "
        "and approximate date, a well-documented spec, a settled definition — do not "
        "demote these to vague 'possibilities' or 'unconfirmed reports' merely because "
        "this session's search came up short; absence of retrieval is not the same as "
        "the fact being uncertain. (2) For anything you genuinely cannot confirm — "
        "exact figures, fast-moving or recent specifics, anything you'd be guessing at "
        "— say so explicitly and decline rather than speculate. The failure to avoid "
        "is BOTH asserting a guess as fact AND timidly hedging a fact you actually "
        "know; a confident known fact plus an honest 'I can't verify the finer details' "
        "beats a wall of hedged maybes."
    )
